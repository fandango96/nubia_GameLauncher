...
public class GifTypeRequest<ModelType> extends GifRequestBuilder<ModelType> {
    ...
    private static <A, R> FixedLoadProvider<A, InputStream, GifDrawable, R> buildProvider(Glide glide, ModelLoader<A, InputStream> streamModelLoader2, Class<R> transcodeClass, ResourceTranscoder<GifDrawable, R> transcoder) {
        if (streamModelLoader2 == null) {
            return null;
        }
        if (transcoder == null) {
            transcoder = glide.buildTranscoder(GifDrawable.class, transcodeClass);
        }
        return new FixedLoadProvider<>(streamModelLoader2, transcoder, glide.buildDataProvider(InputStream.class, GifDrawable.class));
    }

    GifTypeRequest(GenericRequestBuilder<ModelType, ?, ?, ?> other, ModelLoader<ModelType, InputStream> streamModelLoader2, OptionsApplier optionsApplier2) {
        super(buildProvider(other.glide, streamModelLoader2, GifDrawable.class, null), GifDrawable.class, other);
